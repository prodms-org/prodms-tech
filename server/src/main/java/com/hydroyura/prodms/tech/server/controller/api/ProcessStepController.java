...
public class ProcessStepController extends AbstractRestController {

    private final ProcessStepService service;


    @RequestMapping(method = GET, value = "/{process-number}/step/{step-number}")
    public ResponseEntity<ApiRes<?>> get(@PathVariable("step-number") String stepNumber,
                                         @PathVariable("process-number") String processNumber,
                                         HttpServletRequest request) {
        var apiRes = buildEmptyApiResponse(request);
        var result = service.get(stepNumber);

        return result
            .map(apiRes::setData)
            .map(arg -> new ResponseEntity<ApiRes<?>>(arg, HttpStatus.OK))
            .orElseGet(() -> {
                apiRes.getErrors().add(RESPONSE_ERROR_MSG_PROCESS_STEP_NOT_FOUND.formatted(processNumber, stepNumber));
